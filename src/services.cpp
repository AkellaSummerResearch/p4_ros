...
ServicesClass::ServicesClass(ros::NodeHandle *nh) {
	...
	ROS_INFO("[p4_services] Created service %s", trapezoidal_srv.getService().c_str());
...
bool  ServicesClass::trapezoidal_service(p4_ros::trapezoidal_p2p::Request  &req,
	...
	for (uint i = 0; i < time_hist.size(); i++) {
		res.pva_vec.push_back(p4_helper::constuct_PVA(
