...
planner::planner (const Eigen::Vector3d &init_pos,
 		          const Eigen::Vector3d &final_pos,
 		          const double &max_vel, const double &max_acc) {
	init_pos_ = init_pos;
	final_pos_ = final_pos;
	final_displacement_ = (final_pos - init_pos).norm();
	direction_ = (final_pos - init_pos).normalized();
	max_vel_ = max_vel;
	max_acc_ = max_acc;

	if (max_acc <= 0.0) {
		std::cout << "Maxium acceleration less than zero! Setting it to 0.1 m/s^2" << std::endl;
		max_acc_ = 0.1;
	} else {
		max_acc_ = max_acc;
	}
...
void planner::get_pva_at_time (const double &time,
 		Eigen::Vector3d *pos, Eigen::Vector3d *vel, Eigen::Vector3d *acc) {
	// Run through all different cases
	if (time <= 0) {
		*pos = init_pos_;
...
void planner::sample_trajectory (const double &sample_freq,
...
void planner::plot_traj_1D (const std::vector<double> &time_hist,
